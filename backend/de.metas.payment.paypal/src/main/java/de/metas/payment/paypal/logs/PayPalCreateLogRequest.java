...
public class PayPalCreateLogRequest
{
	...
	public static class PayPalCreateLogRequestBuilder
	{
		...
		private static ImmutableMap<String, String> toMap(@NonNull final Headers headers)
		{
			if (headers == null)
			{
			...
			{
				final String headerNorm = CoalesceUtil.coalesce(header, ""); // yes, headers's iterable might return null, so we need to avoid an NPE
				final String value = headers.header(headerNorm); 
