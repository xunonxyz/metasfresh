...
public class GetAmazonOrderBuyerProcessor implements Processor
{
	...
	public void process(Exchange exchange) throws Exception
	{
		...
		log.debug("Get buyer info for amazon order {}", importOrderRouteContext.getOrderNotNull().getAmazonOrderId());
		...
		GetOrderBuyerInfoResponse response = orderApi.getOrderBuyerInfo(order.getAmazonOrderId());

		
		if (CollectionUtils.isEmpty(response.getErrors()))
		{
			importOrderRouteContext.setOrderBuyerInfo(response.getPayload());
		}
		else
		{
