...
public class GetAmazonOrderItemsProcessor implements Processor
{
	...
	public void process(Exchange exchange) throws Exception
	{
		...
		log.debug("Get order items for amazon order {}", importOrderRouteContext.getOrderNotNull().getAmazonOrderId());
		...
		GetOrderItemsResponse response = orderApi.getOrderItems(order.getAmazonOrderId(), "");

		
		if (CollectionUtils.isEmpty(response.getErrors()))
		{
			importOrderRouteContext.setOrderItemsList(response.getPayload());
		}
		else
		{
			...
			throw new RuntimeException("Amazon:Failed to get order order items! " + Instant.now());
		}
	}
